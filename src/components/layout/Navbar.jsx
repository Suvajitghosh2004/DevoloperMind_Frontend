...
export default function Navbar() {
  ...
  // Fetch real categories from DB so slugs always match
  useEffect(() => {
    api.get('/categories')
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-primary/95 backdrop-blur border-b border-border-dark shadow-lg' : 'bg-primary border-b border-border-dark/50'}`}>
      ...
      {/* Category strip — real categories from DB */}
      {categories.length > 0 && (
        <div className="border-t border-border-dark/50 overflow-x-auto">
          ...
          <div className="px-4 py-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="border-t border-border-dark mt-2 pt-2">
                <p className="text-xs text-text-muted px-3 mb-1">Categories</p>
                {categories.filter(c => c.isActive).map(cat => (
                  <Link key={cat._id} to={`/category/${cat.slug}`} onClick={() => setMobileOpen(false)}
                    className="py-1.5 px-3 text-sm text-text-muted hover:text-text-main rounded-lg hover:bg-primary/50 block">
                    {cat.name}
                  </Link>
                ))}
              </div>
